...
def login():
    ...
    if request.method == 'POST':
        ...
        # 将用户输入以POST方式提交给官网
        response = requests.post('http://127.0.0.1:5000/login', data={'username': username, 'password': password}, allow_redirects=False)
        if response.status_code == 302 and 'Location' in response.headers and response.headers['Location'].endswith('/index'):
            ...
            response = make_response(redirect('http://127.0.0.1:5000/index'))
